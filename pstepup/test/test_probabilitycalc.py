...
def test_prob_for_exactly_x():
    ...
    actual = { k:round(v,7) for k,v in probabilitycalc.prob_for_exactly_x(0.02, 4).iteritems() }
    assert actual == { 0 : 0.9223682, 1 : 0.0752954, 2 : 0.002305, 3 : 0.0000314, 4 : 0.0000002 }

    actual = { k:round(v,11) for k,v in probabilitycalc.prob_for_exactly_x(0.01, 7).iteritems() }
...
def test_at_least_x():
    ...
    at_least_x = { k:round(v,5) for k,v in probabilitycalc.at_least_x(prob_exactly_x).iteritems() }
...
def test_aggregating_prob_exactly_x():
    ...
    agg_prob_exactly_x = { k:round(v,5) for k,v in probabilitycalc.aggregate_prob_exactly_x(prob_dict).iteritems() }
    ...
    actual = { k:round(v,5) for k,v in probabilitycalc.aggregate_prob_exactly_x(prob_dict).iteritems() }
